...
{
    [Binding,Scope(Tag = "SoftwareRequirementID_10358")]
    public class Req10358Steps
    {
        ...
        public Req10358Steps(ScenarioContext scenarioContext, IWebDriver driver)
        {
        ...
        public void GivenUserIsOnPage(string reportType)
        {
            ...
            if (reportType.ToLower().Trim().Equals("progressa firmware upgrade status"))
            {
        ...
        public void ThenReportIsDownloadedAsCsvFile(string reportName)
        {
            ...
            {
                fileName = FirmwareStatusReportPageExpectedValues.ProgressaFirmwareStatusReportName;
            }
            else
            {
            ...
            (isFileDownloaded).Should().BeTrue(because: reportName + " file should be downloaded when user clicks on download button in progressa firmware upgrade status report page.");
